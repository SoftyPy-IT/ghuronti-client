    ...
    if (willDelete) {
      try {
        const res = await fetch(
          `http://localhost:5000/api/v1/requirement/hajj/${id}`,
          {
            method: "DELETE",
          }
        );
          ...
          <tbody className="w-full">
            {packages?.map((requirement) => (
              <tr key={requirement._id}>
                <td>  <Image src={requirement.image[0]} alt="" width={100} height={100} className="h-32 w-32"></Image></td>
                <td> {requirement.title}</td>
                {/* <td>Renewal trade license copy with notary public</td> */}
                <td> {requirement.sub_title}</td>
                <td>
                  <Link
                    href={`/b2bdashboard/manage/hajj-visa-requirement/update?id=${requirement._id}`}
                  >
        ...
        <div className="flex items-center justify-between px-8 mb-5">
          ...
          <div className={style.addHotel}>
            <Link href="/b2bdashboard/manage/hajj-visa-requirement/add">
              <button>
                <span className="text-xl font-bold">+</span> Add Hajj Visa
                Requirement{" "}
              </button>
            </Link>
          </div>
...
export async function getStaticProps() {
  try {
    const response = await axios.get(
      "http://localhost:5000/api/v1/requirement/hajj/package"
    );
